...
export default function MeetingRoom() {
  ...
  const sendSignal = useCallback(async (meetingId: string, targetId: string, type: string, payload: unknown) => {
  ...
  const startOffer = useCallback(async (meetingId: string, targetId: string, state: PeerState) => {
    const uid = firebaseAuth.currentUser?.uid;
    if (!uid || uid > targetId || state.offerStarted) return;
    ...
    try {
      ...
      if (!description) throw new Error('Local offer description was not created');
  ...
  const createPeer = useCallback(async (meetingId: string, targetId: string) => {
    ...
    pc.onconnectionstatechange = () => {
      ...
      if (pc.connectionState === 'failed') {
        console.warn('[WebRTC] peer failed; closing', { targetId });
        pc.close();
      }
    ...
    // Initial media is already available before participants are discovered.
    ...
    if (uid < targetId) {
      // Don't rely on onnegotiationneeded for initial setup. Explicitly start the call.
      await startOffer(meetingId, targetId, state);
    }
  ...
  useEffect(() => {
    if (localVideoRef.current && stream && !sharing) localVideoRef.current.srcObject = stream;
  }, [sharing, stream]);
  ...
  useEffect(() => {
    ...
    if (!uid) return;
    let active = true;
    const signalingQuery = query(collection(firestore, `meetings/${meeting.id}/signaling`), where('targetId', '==', uid));
    signalUnsub.current?.();
    signalUnsub.current = onSnapshot(signalingQuery, async (snapshot) => {
      for (const change of snapshot.docChanges()) {
        ...
        try {
          if (data.type === 'offer') {
            // The higher UID is polite and accepts the lower UID's offer.
            ...
            if (!answer) throw new Error('Local answer description was not created');
          ...
          console.error('[WebRTC] signaling handling failed', { type: data.type, targetId: data.senderId, error });
        }
      }
  ...
  const toggle = async (kind: 'audio' | 'video') => {
    try {
      ...
      if (!track) {
        ...
        if (kind === 'audio') setAudio(true); else setVideo(true);
        return;
      }
  ...
  const shareScreen = async () => {
    if (sharing) {
      try { await stopScreenShare(); } catch (error) { console.error('[WebRTC] stop screen share failed', error); }
      return;
    }
    try {
      ...
      setSharing(true);
      track.onended = () => { void stopScreenShare().catch((error) => console.error('[WebRTC] screen ended handler failed', error)); };
    } catch (error: any) {
      setSharing(false);
  ...
  return <main className="relative flex h-screen min-h-0 flex-col overflow-hidden bg-slate-950 text-white">
    <header className="relative z-50 flex h-16 shrink-0 items-center justify-between border-b border-white/10 px-4 sm:px-5"><div className="min-w-0"><p className="truncate font-semibold">{meeting?.title || 'RTC Meeting'}</p><button type="button" onClick={() => void copyInvite()} className="mt-0.5 flex items-center gap-1 text-xs text-slate-400 hover:text-white" aria-label="Copy invite link"><span>{code}</span><Copy size={12}/>{copied && ' Copied'}</button></div><div className="flex items-center gap-3 text-slate-400"><Shield size={16}/><span className="text-xs">{participants.length} participant{participants.length === 1 ? '' : 's'}</span></div></header>
    {isHost && waitingParticipants.length > 0 && <div className="relative z-40 flex shrink-0 items-center justify-between gap-3 border-b border-amber-400/20 bg-amber-500/10 px-4 py-3 text-sm"><div className="min-w-0"><p className="font-medium text-amber-200">{waitingParticipants.length} participant{waitingParticipants.length === 1 ? '' : 's'} waiting for approval</p><p className="truncate text-xs text-amber-100/70">{waitingParticipants.map((participant) => participant.displayName || 'Participant').join(', ')}</p></div><button type="button" onClick={() => { setChat(false); setPeople(true); }} className="shrink-0 rounded-lg bg-amber-300 px-3 py-2 text-xs font-semibold text-slate-950">Review</button></div>}
    <section className="relative z-0 flex min-h-0 flex-1 overflow-hidden"><div className="relative min-w-0 flex-1 overflow-auto p-3 pb-24 sm:p-4 sm:pb-24"><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3"><div className="relative aspect-video overflow-hidden rounded-2xl border border-white/10 bg-slate-900"><video ref={localVideoRef} autoPlay muted playsInline className={`h-full w-full object-cover ${video || sharing ? '' : 'hidden'}`}/>{!video && !sharing && <div className="absolute inset-0 grid place-items-center"><div className="grid h-20 w-20 place-items-center rounded-full bg-slate-800 text-2xl font-semibold">U</div></div>}<span className="absolute bottom-3 left-3 rounded-lg bg-black/50 px-2.5 py-1 text-xs">You</span>{sharing && <span className="absolute right-3 top-3 rounded-lg bg-emerald-500/90 px-2.5 py-1 text-xs font-medium">Screen sharing</span>}</div>{participants.map((participant) => { if (!participant.uid || participant.uid === uid || participant.status === 'waiting') return null; return <div key={participant.uid} className="relative aspect-video overflow-hidden rounded-2xl border border-white/10 bg-slate-900"><video ref={(element) => { remoteRefs.current[participant.uid] = element; if (element) attachRemoteStream(participant.uid); }} autoPlay playsInline className="h-full w-full object-cover"/>{!remoteStreams.current[participant.uid] && <div className="absolute inset-0 grid place-items-center text-sm text-slate-500">Connecting media…</div>}<span className="absolute bottom-3 left-3 rounded-lg bg-black/50 px-2.5 py-1 text-xs">{participant.displayName || 'Participant'}</span></div>; })}</div></div>
      {(chat || people) && <><button type="button" aria-label="Close panel" onClick={() => { setChat(false); setPeople(false); }} className="absolute inset-0 z-[60] bg-black/40 md:hidden"/><aside className="absolute inset-y-0 right-0 z-[70] flex w-[min(90vw,24rem)] flex-col border-l border-white/10 bg-slate-900 shadow-2xl md:relative md:z-20 md:w-80 md:shadow-none"><div className="flex shrink-0 items-center justify-between border-b border-white/10 p-4"><h2 className="font-semibold">{chat ? 'Chat' : 'Participants'}</h2><button type="button" onClick={() => { setChat(false); setPeople(false); }} aria-label="Close" className="grid h-9 w-9 place-items-center rounded-lg hover:bg-white/10"><X size={18}/></button></div>{chat ? <><div className="min-h-0 flex-1 space-y-3 overflow-y-auto p-4">{messages.length ? messages.map((message) => <div key={message.id}><p className="text-xs text-slate-500">{message.senderName || 'Participant'}</p><p className="mt-1 break-words rounded-xl bg-white/5 px-3 py-2 text-sm">{message.deletedAt ? '[Message deleted]' : message.content}</p></div>) : <p className="mt-8 text-center text-sm text-slate-500">No messages yet.</p>}</div><div className="shrink-0 border-t border-white/10 bg-slate-900 p-3 pb-4"><div className="flex gap-2"><input value={messageText} onChange={(event) => setMessageText(event.target.value)} onKeyDown={(event) => event.key === 'Enter' && void sendMessage()} className="min-w-0 flex-1 rounded-xl bg-white/5 px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-indigo-500" placeholder="Type a message…"/><button type="button" onClick={() => void sendMessage()} className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-indigo-500" aria-label="Send message"><Send size={16}/></button></div></div></> : <div className="min-h-0 flex-1 space-y-2 overflow-y-auto p-4">{participants.length === 0 && <p className="text-sm text-slate-500">No participants found.</p>}{participants.map((participant) => <div key={participant.uid} className="rounded-xl bg-white/5 p-3"><div className="flex items-center gap-3"><div className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-indigo-500/30 text-sm font-semibold">{(participant.displayName || 'U').charAt(0).toUpperCase()}</div><div className="min-w-0 flex-1"><p className="truncate text-sm">{participant.displayName || 'User'}</p><p className="text-xs text-slate-500">{participant.role === 'host' ? 'Host' : participant.status === 'waiting' ? 'Waiting for approval' : participant.status === 'active' ? 'Participant' : participant.status}</p></div></div>{isHost && participant.role !== 'host' && participant.status === 'waiting' && <div className="mt-3 grid grid-cols-2 gap-2"><button type="button" onClick={() => void moveParticipant(participant.uid, true)} className="min-h-10 rounded-lg bg-emerald-500 px-3 py-2 text-xs font-bold text-slate-950">Approve</button><button type="button" onClick={() => void moveParticipant(participant.uid, false)} className="min-h-10 rounded-lg bg-red-500 px-3 py-2 text-xs font-bold text-white">Deny</button></div>}</div>)}</div>}</aside></>}
    </section>
    <footer className="absolute bottom-0 left-0 right-0 z-[80] flex min-h-20 items-center justify-center gap-2 border-t border-white/10 bg-slate-950/95 px-2 py-3 backdrop-blur sm:gap-3 sm:px-4"><button type="button" onClick={() => void toggle('audio')} aria-label={audio ? 'Mute microphone' : 'Unmute microphone'} className={`grid h-12 w-12 shrink-0 place-items-center rounded-full ${audio ? 'bg-slate-800 hover:bg-slate-700' : 'bg-red-500 hover:bg-red-400'}`}>{audio ? <Mic/> : <MicOff/>}</button><button type="button" onClick={() => void toggle('video')} aria-label={video ? 'Turn camera off' : 'Turn camera on'} className={`grid h-12 w-12 shrink-0 place-items-center rounded-full ${video ? 'bg-slate-800 hover:bg-slate-700' : 'bg-red-500 hover:bg-red-400'}`}>{video ? <Video/> : <VideoOff/>}</button><button type="button" onClick={() => void shareScreen()} aria-label={sharing ? 'Stop sharing' : 'Share screen'} className={`grid h-12 w-12 shrink-0 place-items-center rounded-full ${sharing ? 'bg-indigo-500' : 'bg-slate-800 hover:bg-slate-700'}`}><MonitorUp/></button><button type="button" onClick={() => void copyInvite()} aria-label="Share meeting link" className="grid h-12 w-12 shrink-0 place-items-center rounded-full bg-slate-800 hover:bg-slate-700"><Share2/></button><button type="button" onClick={() => { setPeople(false); setChat((value) => !value); }} aria-label="Chat" className={`grid h-12 w-12 shrink-0 place-items-center rounded-full ${chat ? 'bg-indigo-500' : 'bg-slate-800 hover:bg-slate-700'}`}><MessageSquare/></button><button type="button" onClick={() => { setChat(false); setPeople((value) => !value); }} aria-label="Participants" className={`relative grid h-12 w-12 shrink-0 place-items-center rounded-full ${people ? 'bg-indigo-500' : 'bg-slate-800 hover:bg-slate-700'}`}><Users/>{isHost && waitingParticipants.length > 0 && <span className="absolute -right-0.5 -top-0.5 grid h-5 min-w-5 place-items-center rounded-full bg-amber-400 px-1 text-[10px] font-bold text-slate-950">{waitingParticipants.length}</span>}</button><button type="button" onClick={() => void leave()} aria-label="Leave meeting" className="ml-1 grid h-12 w-14 shrink-0 place-items-center rounded-full bg-red-500 hover:bg-red-400"><PhoneOff/></button></footer>
  </main>;
}
